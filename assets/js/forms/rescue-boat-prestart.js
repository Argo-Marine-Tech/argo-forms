(() => {
  ...
  function normalizeChecklistResult(value) {
    const normalized = String(value ?? '').trim().toLowerCase();

    if (['yes', 'y', 'true', '1', 'pass', 'passed', 'satisfactory'].includes(normalized)) {
      return 'Yes';
    }

    if (['no', 'n', 'false', '0', 'fail', 'failed', 'unsatisfactory'].includes(normalized)) {
      return 'No';
    }
  ...
  function collectChecklist() {
    return [...form.querySelectorAll('.rescue-item-row')].map((row) => {
      ...
      return {
        ...
        result: normalizeChecklistResult(checkedInput?.value),
  ...
  function createRadioMarker(selected) {
    const marker = createElement(
      'span',
      selected ? 'rescue-pdf-radio is-selected' : 'rescue-pdf-radio'
    );
  ...
  function createItemRow(item, commentText) {
    ...
    const normalizedResult = normalizeChecklistResult(item.result);

    const yes = createElement('td', 'rescue-pdf-choice');
    yes.appendChild(createRadioMarker(normalizedResult === 'Yes'));
    row.appendChild(yes);

    const no = createElement('td', 'rescue-pdf-choice');
    no.appendChild(createRadioMarker(normalizedResult === 'No'));
    row.appendChild(no);
  ...
  function prepareSection(section, checklistByNumber, detailCounter) {
    ...
    const items = section.items.map(([number, fallbackText]) => {
      ...
      const item = {
        ...
        result: normalizeChecklistResult(response.result),
