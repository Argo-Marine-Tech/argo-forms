(() => {
  ...
  function replaceControlsForPdf(sourcePage, clonedPage) {
    ...
    clonedControls.forEach((control, index) => {
      ...
      if (source.type === 'radio') {
        ...
        marker.className = `pdf-radio${source.checked ? ' is-checked' : ''}`;
      ...
      display.className = source.tagName === 'TEXTAREA'
        ? 'pdf-field-value pdf-comment-value'
        : 'pdf-field-value';
  ...
  async function waitForCloneAssets(root) {
  ...
  async function createPdfBase64() {
    ...
    exportHost.className = 'rescue-pdf-export-host';
    ...
    try {
      for (let index = 0; index < sourcePages.length; index += 1) {
        ...
        clonedPage.classList.add('rescue-pdf-export-page');
        clonedPage.removeAttribute('id');
        clonedPage.querySelectorAll('[id]').forEach((element) => element.removeAttribute('id'));
        replaceControlsForPdf(sourcePage, clonedPage);
        exportHost.replaceChildren(clonedPage);
        await waitForCloneAssets(clonedPage);
