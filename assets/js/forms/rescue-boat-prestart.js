(() => {
  ...
  async function createPdfBase64() {
    if (!window.html2canvas || !window.jspdf?.jsPDF) throw new Error('PDF libraries did not load. Refresh the page and try again.');
    const { jsPDF } = window.jspdf;
    const pdf = new jsPDF({ orientation:'portrait', unit:'mm', format:'a4', compress:true });
    const pages = [...document.querySelectorAll('.rescue-print-page')];
    for (let index=0; index<pages.length; index += 1) {
      const canvas = await window.html2canvas(pages[index], { scale:1.5, useCORS:true, backgroundColor:'#ffffff', logging:false });
