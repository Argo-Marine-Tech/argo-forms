(() => {
  ...
  const insuranceDocumentInput = document.getElementById('insuranceDocument');
  ...
  const supportingDocumentSpecs = Object.freeze([
    ...
    { input: insuranceDocumentInput, documentType: 'Insurance Document', requiredDocument: true },
  ...
  function validateSupportingDocuments(showMessage = true) {
    ...
    if (!message && !fileFromInput(insuranceDocumentInput)) message = 'A current insurance document is required for every berth application.';
    ...
    insuranceDocumentInput.setCustomValidity(!fileFromInput(insuranceDocumentInput) ? 'Current insurance document required.' : '');
    ...
    if (showMessage) {
      ...
      document.getElementById('insurance-document-block')?.classList.toggle('is-invalid-group', !fileFromInput(insuranceDocumentInput));
  ...
  form.addEventListener('change', (event) => {
    ...
    if ([manifestInput, insuranceDocumentInput, catchReportInput, hotWorksPermitInput, crewListInput, supportingDocumentsInput].includes(event.target)) renderSupportingDocuments();
